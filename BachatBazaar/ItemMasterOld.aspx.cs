...
{
    public partial class ItemMasterOld : System.Web.UI.Page
    {
        SqlDataAdapter da;
        DataSet ds;
        DataRow drw;
            ...
            try
            {
                using (var cnt = ConnectionManager.Inst.CreateNewConnection())
                {
                    ...
                    if (txtFrontPosition.Text.Trim().Length > 0)
                    {
                        ...
                        {
                            ssql = "select * from ItemMaster where ShowFrontTop_3=1 and UserName='"+strUser+"' and  FrontPosition=" + Convert.ToInt32(txtFrontPosition.Text) + "";
                            boolFront = true;
                        }
                        if (chkIsFrontMiddle.Checked == true)
                        {
                            ssql = "select * from ItemMaster where ShowFrontMid_3=1 and UserName='" + strUser + "' and FrontPosition=" + Convert.ToInt32(txtFrontPosition.Text) + "";
                            boolFront = true;
                        }
                        if (chkIsFrontBottom3.Checked == true)
                        {
                            ssql = "select * from ItemMaster where ShowFrontBottom_3=1 and UserName='" + strUser + "' and FrontPosition=" + Convert.ToInt32(txtFrontPosition.Text) + "";
                            boolFront = true;
                        }
                        if (chkIsFrontLeftVerticle.Checked == true)
                        {
                            ssql = "select * from ItemMaster where ShowLeftFront_5=1 and UserName='" + strUser + "' and FrontPosition=" + Convert.ToInt32(txtFrontPosition.Text) + "";
                            boolFront = true;
                        }
                        if (chkIsFrontPageSlider.Checked == true)
                        {
                            ssql = "select * from ItemMaster where ShowFrontSlider_10=1 and UserName='" + strUser + "' and FrontPosition=" + Convert.ToInt32(txtFrontPosition.Text) + "";
                            boolFront = true;
                        }
