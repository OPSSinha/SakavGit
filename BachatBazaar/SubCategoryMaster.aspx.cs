...
namespace BachatBazaar
{
    public partial class SubCategoryMaster : System.Web.UI.Page
    {
        ...
        //    string strUser = Session["UserName"].ToString();
        //    Int32 k = 0;
        //    DataTable dt = MDC.getDataTable("select * from Individual_App1 where UserName='" + strUser + "'");
        //    if (dt.Rows.Count > 0)
        //    {
        //        foreach (DataRow dr in dt.Rows)
        //        {

        //            drpCategory.Items.Add(dt.Rows[k]["Category"].ToString());
        ...
        protected void gvMenuTiming_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Show")
            {
                ...
                using (var cnt = ConnectionManager.Inst.CreateNewConnection())
                {
                    //    SqlDataAdapter daSql = new SqlDataAdapter("select * from Individual_App1 where PK_APP_ID=" + Convert.ToInt32(dealId), cnt);
